...
| `scan` | — | project-documentation pass: discover the local argosy from `argosy://_argosys`, check existing documents via `search` (`namespaces: ["document"]`) + `read_memory`, investigate the project with the harness's own file tools (plus `repomap`/`outline`/`inspect` when the server exposes code tools), then write the core set — `document/summary`, `document/architecture`, `document/tech`, `document/development` — via `write_document` (frontmatter `type` required, DOC-1), with optional `document/decisions/<slug>`, `document/glossary`, `document/conventions` only when warranted; rules ground every claim in files actually read, distill rather than copy canonical docs, update in place on re-runs (never near-duplicates), never touch read-only imported argosys, modify no project files, and end with a one-paragraph report |
...
- [ ] Prompt coverage: wire test asserts `list_prompts` returns exactly `dream` and `scan`, `get_prompt` returns one user-role message naming each workflow's tools (`dream`: `search`, `read_memory`, `write_memory`, `delete_memory`; `scan`: the read/search tools plus `write_document` / `delete_document` and the core document paths), and an unknown prompt name is a protocol error.

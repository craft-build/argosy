...
| Default embedding provider | **`tract`** (pure-Rust `tract-onnx`, local — no C in the dependency chain) behind the `EmbeddingProvider` trait, running the ONNX export of `all-MiniLM-L6-v2`; model identity recorded per `IDX-5`. Supersedes the `candle` resolution as of 2026-09-23 (doc 12), which itself superseded the original `fastembed` (ONNX) resolution (doc 11) |
...
│   └── tract.rs         # TractProvider   (doc 12, feature "default-index")
...
Cargo features: `default = ["default-index", "mcp"]`; `default-index` gates the sqlite-vec/tract backend; `mcp` gates `rmcp` so library consumers can stay lean.
...
- **Dependencies.** Keep the tree minimal; every dependency must be justified in the document that introduces it. Already prescribed: `thiserror`, `serde` + `serde_yaml` (frontmatter), `semver` (manifest), `tempfile` (dev). Docs 07/09/10 add `rusqlite`/`sqlite-vec`/`fastembed`, `clap`, and `rmcp` respectively; doc 11 replaces the fastembed entry with the pure-Rust `candle`/`hf-hub`/`tokenizers` stack; doc 12 replaces the candle runtime with `tract-onnx` (same `hf-hub`/`tokenizers` stack).
- **Testing.** Unit tests colocated with the code they test; fixtures under `tests/fixtures/` constructed as real directory trees (write small helper builders as needed). `cargo test` with default features must require **no network access** — the embedding model download is exercised only in a gated test (docs 07/11/12).
...
| 12 | `12-tract-embedding-backend.md` | `TractProvider`: pure-Rust tract-onnx embedding backend replacing candle, running the ONNX export | 06 |

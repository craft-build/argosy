...
/// The initial hash value: first 32 bits of the fractional parts of the
...
pub(crate) fn sha256_hex(data: &[u8]) -> String {
    ...
    for chunk in msg.chunks_exact(64) {
        let mut w = [0u32; 64];
        for (i, w4) in w[..16].iter_mut().enumerate() {
            *w4 = u32::from_be_bytes([
                chunk[4 * i],
                chunk[4 * i + 1],
                chunk[4 * i + 2],
                chunk[4 * i + 3],
...
mod tests {
    ...
    fn matches_the_standard_test_vectors() {
        let cases: [(&[u8], &str); 5] = [
            ...
            // 55 bytes: lands exactly at the padding offset once the `0x80`
            // is appended (56 bytes), so the length fits in the same block.
            (
                &[b'a'; 55],
                "9f4390f8d30c2dd92ec9f095b65e2b9ae9b0a925a5258e241c9f1e910f734318",
            ),
            // 56 bytes (the FIPS 180-4 example): the `0x80` plus the length
            // spill into a second padding block — the path an off-by-one in
            // the padding loop would break.
            (
    ...
    #[test]
    fn matches_the_vector_for_many_blocks() {
        // One million `a`s: ~15k blocks through `chunks_exact`, the other
        // FIPS 180-4 known-answer vector, pinning multi-block behavior.
